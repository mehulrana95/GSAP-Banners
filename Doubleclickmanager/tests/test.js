...
describe('zipfile', function() {
	it('should be under 200kb', function() {

		var zip = fs.statSync('doubleclickmanager.zip');
		var fileSizeInKB = zip['size']/1000;
		expect(fileSizeInKB).not.toBeGreaterThan(200, '\nZip size is ' + fileSizeInKB + 'kb, it should be less than 200kb');
	});
});
	...
	function assetList(dir, filelist) {
		...
		files.forEach(function(file) {
			if (fs.statSync(dir + file).isDirectory()) {
				...
				if (!/\.DS\_Store/.test(file)) filelist.push(file);
			}
		}); 
